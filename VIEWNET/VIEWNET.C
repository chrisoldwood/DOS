...
** (C) Chris Wood 1995.
**
** VIEWNET.C - Main entry/init/exit point for application.
**
******************************************************************************* 
*/
...
extern void ScanMenu(void);
extern void ReadCfgFile(char *);
...
void Initialise(char *ExePath)
{
     ...
     /* Initialise IPX subsystem, must be first IPX call. */
     uIPXInitCode = IPXInitialize();
     if (uIPXInitCode)
     {
          printf("Error (%x): Can't initialise IPX.\n", (WORD) uIPXInitCode);
          exit(EXIT_FAILURE);
     }
     ...
     /* Read in the node/name mappings. */
     ReadCfgFile(ExePath);
